...
def tool_newref(args):
    ...
    for infile in args.infiles:
        ...
        from wisecondorX.overall_tools import scale_sample
        samples.append(scale_sample(sample, binsize, args.binsize))

    samples = np.array(samples)
    from wisecondorX.newref_tools import train_gender_model
    genders, trained_cutoff = train_gender_model(samples)

    if not args.nipt:
        from wisecondorX.overall_tools import gender_correct
        for i, sample in enumerate(samples):
            samples[i] = gender_correct(sample, genders[i])

    from wisecondorX.newref_tools import get_mask
...
def tool_test(args):
    ...
    if not ref_file['is_nipt']:
        from wisecondorX.predict_tools import predict_gender
    ...
    from wisecondorX.predict_tools import log_trans
    log_trans(results, m_lr)

    if args.blacklist:
        logging.info('Applying blacklist ...')
        from wisecondorX.predict_tools import apply_blacklist
    ...
    if args.plot:
        ...
        from wisecondorX.predict_output import exec_write_plots
