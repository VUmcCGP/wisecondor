...
def getYPerc(sample, testChrom):
    return sum(sample[testChrom])/sum([sum(sample[chrom]) for chrom in sample])


def getYPercMean(sampleList, testChrom):
    values=[]
    for sample in sampleList:
        values.append(getYPerc(sampleList[sample], testChrom))
...
def getYPercGrand(sampleList, testChrom):
    values=[]
    for sample in sampleList:
        values.append(getYPerc(sampleList[sample], testChrom))
...
def defrag(args):
    ...
    percYBoys = getYPercMean(boySamplesPickle, testChrom)
    percYGirls = getYPercMean(girlSamplesPickle, testChrom)
    
    if args.maledir:
        percYMales = getYPercMean(maleSamplesPickle, testChrom)
        maleCorMedian = []
    ...
    training = getYPercGrand(girlSamplesPickle, testChrom)[:]
    training.extend(getYPercGrand(boySamplesPickle, testChrom)[:])
    training = np.array([[x] for x in training])
    targets = [0] * len(getYPercGrand(girlSamplesPickle, testChrom))
    targets.extend([1] * len(getYPercGrand(boySamplesPickle, testChrom)))
    ...
    for index,testSample in enumerate(sortedList):
        ...
        prediction = gnb.predict(getYPerc(testSamplesPickle[testSample], testChrom))
    
        #Based on: %chrY sample = meanY% males * FF + meanY% women with female fetusses * (1 - FF)
        daphGender = solveFetalFraction(percYMales, percYGirls, getYPerc(testSamplesPickle[testSample], testChrom))
        ...
        lines = [testSample.split('/')[-1], str((median(result)/corrMalesMedian)*100), str(daphGender*100), str(getGender(prediction)), str(getCoverage(testSamplesPickle[testSample])), cluster, str(getYPerc(testSamplesPickle[testSample], testChrom)*100)]
        print "\t".join(lines)
        ...
        ax2.set_ylim(0,0.05)
        ax2.scatter(0, getYPerc(testSamplesPickle[testSample], testChrom)*100, c=color, marker='o')
